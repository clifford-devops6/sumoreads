   ...
   <reader>
    ...
    <div class="px-3">
            ...
            <div v-for="(article, index) in chunk(articles.data,chunker)" :key="index" >
                <div class="grid grid-cols-2 gap-2">
                    <div @click="submitPost(post.id)" v-for="post in article" :key="post.id"
                         :class="{'bg-gray-200':post.read_status}">
                        <div
                            ...
                            <div class="col-span-2 grid bg-cover bg-center bg-black"
                                 :style="{ 'background-image': 'url(' + post.article.image + ')' }">
                            ...
                            <div class="col-span-4 grid p-1 py-2 relative">
                                ...
                                <div class="absolute bottom-2 flex justify-end w-full px-2 gap-3">
                                    <p class="text-sm"><span
                                        class="text-primary-100 font-semibold">{{ post.article.source.name }}</span> |
                                        <span>{{ post.article.category.name }}</span></p>
        ...
        <div v-else class="my-16 flex justify-center">
            <div>
                <h1 class="text-xl font-bold">You have no articles in your read list!</h1>
                <div class="my-2 flex justify-center">
                    <Link :href="route('news.index')" class="btn-primary"><span class="mr-2"><i class="fa-light fa-house"></i></span>Go home</Link>
                </div>
            </div>
        </div>
        <div>
