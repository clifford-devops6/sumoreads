<template layout="auth">
    ...
    <div class="flex place-content-center h-screen">
        <div class="bg-white shadow-xl rounded-xl self-center md:w-2/5 p-4">
            ...
            <div class="text-center font-bold text-lg mt-5">Reset Password</div>
            <div class="mt-10 px-14">
                <form @submit.prevent="form.post(route('request.store'))">
                    <div class="mt-5">
                    ...
                    <div class="grid mt-4">
                        <button type="submit" class="btn-primary  m-1">Request update <span v-if="form.processing" class="animate-ping"><i class="fa-regular fa-ellipsis"></i></span></button>
                    </div>
                    <div class="py-3" v-if="$page.props.status">
                        <p class="text-sm text-green-600" ><span class="ml-2 text-teal-900"><i class="fas fa-info-circle"></i></span> {{$page.props.status}}</p>
                    </div>
